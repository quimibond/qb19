...
- `qb_obligation`: obligaciones vivas (qué, quién, sobre qué documento, cómo se prueba, cuándo vence). Nacen del correo (memoria) o a mano, viven como **actividades nativas** de Odoo sobre su documento o contacto (hecha = acuse, cancelada = descarte, evidencia las cierra sola), escalan a Dirección y mandan un recordatorio diario. La cobranza NO vive aquí (v3.0.0). **Desde 3.2.0 el espejo en actividades está apagado** (`res.company.obligation_activity_mirror`, default False; la migración quitó las actividades existentes): las obligaciones viven solo en la app Obligaciones. API `create_candidate`. README propio.
...
- `quimibond_nomina`: lo que le falta a `l10n_mx_hr_payroll_account_edi` para timbrar la nómina de Quimibond: registro patronal IMSS por contrato (`hr.version`, la compañía como respaldo) en `nomina12:Emisor`, `SalarioDiarioIntegrado`/`SalarioBaseCotApor` tomados del recibo por código (`INT_DAY_WAGE_BASE`, `INT_DAY_WAGE`) un **centinela de reglas** (SHA-256 de condición y fórmula, cron diario, avisa y no revierte) y el nodo **`HorasExtra`** de la percepción 019 (`Dias` sale de la entrada `HE_DIAS` que captura RH y, si falta, se estima; la herencia de la plantilla se configura sola al instalar y se apaga si Odoo ya emite el nodo). `NumEmpleado` = referencia de empleado con prefijo de nómina (`S-1`, `Q-14`, `C-26`); es requerido en Nómina 1.2, así que sin referencia el módulo detiene el CFDI con `UserError`. Corte de nómina: 1-ene-2027. **Nunca validar ni timbrar en producción.** README propio.
- `qb_memoria`: pestaña Memoria del contacto (Supabase). **Desde 1.3.0 la pestaña está oculta**: solo la ve el grupo `qb_memoria.group_memoria_tab` (privilegio Memoria, sin miembros); el resto del módulo sigue. Desde 1.2.0 muestra la **ficha consolidada** (RPC `memoria_brief`): quién atiende a la empresa, hechos con vigencia y conversaciones resumidas por Claude con estado y pendientes. Además **dueños aprendidos**: cron nocturno que lee la vista `memoria_encargados` (buzón que atiende a cada empresa / área) y lo escribe en el contacto; personas detrás de buzones compartidos en Contactos → Configuración → Buzones (memoria). README propio.
...
- **Un módulo no hereda sus propias vistas.** Al cargar el padre nuevo, Odoo revalida las vistas heredadas que **ya están en la base** (versión anterior) antes de llegar al archivo que las corrige. Si una pestaña o campo del padre desaparece y un hijo del mismo módulo le hacía xpath, el build revienta con «no puede ser localizado en la vista padre» aunque el código nuevo esté bien (54.0.0 y 54.1.0 del SGI, 2026-09-25). La herencia es para extender vistas de OTROS módulos; la ficha propia se escribe completa en un solo `<record>`. Si una herencia propia ya existe en producción y hay que cambiar el padre: `migrations/<version>/pre-migrate.py` que la borra (ir_ui_view + ir_model_data). El checker (`check_odoo_views.py --base-ref`) compara las anclas de la herencia vieja contra el padre nuevo.
