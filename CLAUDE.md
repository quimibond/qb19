...
- `quimibond_nomina`: lo que le falta a `l10n_mx_hr_payroll_account_edi` para timbrar la nómina de Quimibond: registro patronal IMSS por contrato (`hr.version`, la compañía como respaldo) en `nomina12:Emisor`, `SalarioDiarioIntegrado`/`SalarioBaseCotApor` tomados del recibo por código (`INT_DAY_WAGE_BASE`, `INT_DAY_WAGE`) un **centinela de reglas** (SHA-256 de condición y fórmula, cron diario, avisa y no revierte) y el nodo **`HorasExtra`** de la percepción 019 (`Dias` sale de la entrada `HE_DIAS` que captura RH y, si falta, se estima; la herencia de la plantilla se configura sola al instalar y se apaga si Odoo ya emite el nodo). `NumEmpleado` sólo con la referencia de empleado capturada. Corte de nómina: 1-ene-2027. **Nunca validar ni timbrar en producción.** README propio.
