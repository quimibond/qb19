...
- **Vista padre que cambia de forma**: al cargar el padre nuevo, Odoo revalida las vistas heredadas que **ya están en la base** (versión anterior) antes de llegar al archivo que las corrige. Si una pestaña o campo del padre desaparece y un hijo le hacía xpath, el build revienta con «no puede ser localizado en la vista padre» aunque el código nuevo esté bien. Solución: `migrations/<version>/pre-migrate.py` que borra esas vistas heredadas (ir_ui_view + ir_model_data); se recrean solas al cargar los XML (54.0.0 del SGI).
