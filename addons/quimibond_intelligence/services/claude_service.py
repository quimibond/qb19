...
class ClaudeService:
    ...
    def analyze_account_full(self, department: str, account: str,
        ...
        system = (
            'Eres un analista de inteligencia empresarial para Quimibond '
            '(manufacturera de no tejidos y textiles en México). '
            'Realiza DOS análisis en una sola respuesta. '
            'Retorna SOLO JSON válido sin markdown.\n\n'
            'CONTEXTO ODOO: Los emails contienen etiquetas [ODOO: ...] con '
            'datos en vivo del ERP. CRUZA la info del email con Odoo:\n'
            '- Facturas vencidas + pedido nuevo → señalar riesgo\n'
            '- Pregunta de entrega + picking retrasado → señalar\n'
            '- Oportunidad CRM abierta → mencionar revenue\n'
            '- LTV y tendencia → detectar churn si baja\n'
            '- Cartera vencida, devoluciones, OTD bajo → alertar'
        )

        prompt = (
            f'Analiza los {ext_count + int_count} emails de {department} ({account}).\n'
            f'{ext_count} externos, {int_count} internos.\n\n'
            'Retorna UN SOLO JSON con esta estructura:\n'
            '{\n'
            '  "summary": {\n'
            '    "summary_text": "Resumen narrativo cruzando emails con Odoo",\n'
            '    "overall_sentiment": "positive|neutral|negative|mixed",\n'
            '    "sentiment_score": 0.0,\n'
            '    "sentiment_detail": "Breve explicación del tono",\n'
            '    "key_items": [{"item": "desc", "priority": "high|medium|low", '
            '"from": "remitente", "action_needed": "qué hacer", '
            '"importance_score": 1, '
            '"odoo_context": "datos del ERP si aplica"}],\n'
            '    "waiting_response": [{"contact": "nombre <email>", '
            '"subject": "asunto", "hours_waiting": 0, "is_external": true, '
            '"urgency": "high|medium|low", '
            '"business_impact": "impacto si no se responde"}],\n'
            ...
            '"financial_impact": "monto en riesgo"}],\n'
            '    "external_contacts": [{"name": "nombre", "email": "email", '
            '"company": "empresa", "topic": "tema", '
            '"sentiment": "positive|neutral|negative", '
            '"sentiment_score": 0.0, '
            '"relationship_signal": "strengthening|stable|cooling|at_risk", '
            '"odoo_profile": "situación en ERP"}],\n'
            '    "competitors_mentioned": [{"name": "competidor", '
            '"context": "contexto", "threat_level": "high|medium|low", '
            '"mentioned_by": "contacto", "detail": "detalle"}],\n'
            '    "topics_detected": [{"topic": "nombre", '
            '"status": "new|ongoing|resolved", "detail": "desc"}],\n'
            '    "attachment_insights": [{"filename": "nombre", '
            '"type": "pdf|excel|text|image", '
            '"summary": "resumen", "business_impact": "relevancia"}],\n'
            '    "risks_detected": [{"risk": "desc", '
            ...
            '    "person_insights": [{"name": "nombre", '
            '"email": "email", "company": "empresa", '
            '"role_detected": "rol inferido", '
            '"communication_style": "formal|informal|técnico|ejecutivo", '
            '"personality_traits": ["rasgos"], '
            '"key_interests": ["intereses"], '
            '"decision_power": "high|medium|low", '
            ...
            '  "knowledge_graph": {\n'
            '    "entities": [{"name": "str", '
            '"type": "person|company|product|machine|raw_material", '
            '"email": "str or null", "attributes": {}}],\n'
            '    "facts": [{"entity_name": "str", '
            '"type": "commitment|statement|price|quantity|delivery_date|payment|'
            'complaint|request|approval|rejection|information|change", '
            '"text": "str", "date": "YYYY-MM-DD or null", '
            '"is_future": false, "confidence": 0.8}],\n'
            '    "action_items": [{"assignee": "quien ejecuta", '
            '"related_to": "contacto involucrado", '
            '"description": "acción específica", '
            '"reason": "por qué es necesaria", '
            '"type": "call|email|meeting|follow_up|send_quote|send_invoice|'
            'review|approve|deliver|pay|investigate|other", '
            '"priority": "low|medium|high|critical", '
            '"due_date": "YYYY-MM-DD or null"}],\n'
            '    "relationships": [{"entity_a": "str", "entity_b": "str", '
            '"type": "works_at|buys_from|sells_to|manages|supplies|'
            'manufactures|negotiates_with|mentioned_with|reports_to|'
            'collaborates_with", "context": "str"}],\n'
            '    "person_profiles": [{"name": "str", "email": "str or null", '
            '"company": "str or null", "role": "cargo inferido", '
            '"department": "str or null", '
            '"decision_power": "high|medium|low", '
            '"communication_style": "formal|informal|tecnico|ejecutivo", '
            '"language_preference": "es|en|mixed", '
            '"key_interests": ["temas"], '
            '"personality_notes": "observaciones", '
            '"negotiation_style": "aggressive|collaborative|passive|'
            'analytical or null", '
            ...
            '- facts: solo info EXPLICITA. Confidence 0.8+ claros, 0.3-0.5 implícitos\n'
            '- person_profiles en knowledge_graph: perfil COMPLETO de cada persona\n'
            '- person_insights en summary: perfil resumido para el briefing\n'
            '- Detecta competidores, directos e indirectos\n'
            '- CRUZA emails con datos de Odoo [ODOO: ...]\n\n'
            f'EMAILS:\n{email_text}'
        )

        text = self._call(system, prompt, max_tokens=10000)
        parsed = self._extract_json(text)
