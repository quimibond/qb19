...
Lo que queda es lo que usan el push (upsert, insert, rpc) y el pull
(fetch, patch). qb_memoria y quimibond_sat traen su propio cliente.
"""
import json
import logging
import httpx
...
class SupabaseClient:
    ...
    def insert(self, table: str, rows: list, batch_size: int = 200) -> int:
        """Plain INSERT (no upsert) with retry. Used for pipeline_logs."""
        if not rows:
            return 0
        synced = 0
        for i in range(0, len(rows), batch_size):
    ...
    def rpc(self, function: str, params: dict):
        ...
        Catches exceptions and returns None on error, so a transient failure
