...
BEGIN
  ...
  WITH r AS (
    ...
    SET resolved_at = now(),
        resolution = 'auto_syntage_complemento_received',
        resolution_notes = format('payment syntage_invoice_payments.id=%s', p_payment_id)
...
BEGIN
  WITH r AS (
    UPDATE public.reconciliation_issues ri
    SET resolved_at = now(),
        resolution = 'auto_odoo_payment_linked',
        resolution_notes = format('payment linked to invoice_id=%s', p_invoice_id)
    WHERE ri.resolved_at IS NULL
...
BEGIN
  WITH r AS (
    ...
    SET resolved_at = now(),
        resolution = 'auto_odoo_payment_linked',
        resolution_notes = 'backfill phase_1'
    WHERE ri.resolved_at IS NULL
      AND ri.issue_type = 'complemento_missing_payment'
      AND EXISTS (
...
BEGIN
  WITH r AS (
    UPDATE public.reconciliation_issues ri
    SET resolved_at = now(),
        resolution = 'auto_cancellation_reconciled',
...
BEGIN
  WITH r AS (
    UPDATE public.reconciliation_issues ri
    SET resolved_at = now(),
        resolution = 'auto_amount_reconciled',
        resolution_notes = 'totals converge within $0.01'
    WHERE ri.resolved_at IS NULL
